...
{
	public:
		Carte(unsigned int** id_tiles, const unsigned int taille_x, const unsigned int taille_y, const bool collision, const string& file_tileset, SDL_Renderer* renderer) : offset_x(0), offset_y(0)
		{
			largeur = taille_x;
			hauteur = taille_y;
			...
			for(unsigned int i = 0; i < taille_x; i++) {
				...
				for(unsigned int j = 0; j < taille_y; j++) {
					Tile tile_courante(id_tiles[i][j], collision);
